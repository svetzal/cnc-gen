class ToolHead {
    ...
    prepare(depth) {
        ...
        this.writer.write([
            this.gen.createRapidMove(
                this.position.setZ(this.settings.retractHeight)
            ),
            this.gen.createCuttingMove(
                this.position.setZ(depth),
                this.settings.stepDownFeedRate
            ),
        ]);
    }
    ...
    repositionTo(position) {
        this.writer.write([
            ...
            this.gen.createRapidMove(
                position.setZ(this.settings.retractHeight)
            ),
            this.gen.createCuttingMove(
                position.setZ(0),
                this.settings.normalCutFeedRate
            ),
            this.gen.createCuttingMove(
                position,
                this.settings.stepDownFeedRate
    ...
    cutTo(position, feedRate) {
        this.writer.write(this.gen.createCuttingMove(position, feedRate));
        this.position = position;
    }
