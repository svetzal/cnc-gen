...
function helical(toolHead, diameter, depth, force = true) {
    ...
    if (diameter < toolHead.millSize)
        throw "Unable to drill a hole smaller than the end mill diameter";
    if (force && diameter > toolHead.millSize * 2)
        throw "Unable to drill a clean hole larger than twice the end mill diameter";
    ...
    for (; start > targetZ; start -= toolHead.settings.helicalStepDownRate) {
        ...
        else remainingStepdown = toolHead.settings.helicalStepDownRate;
